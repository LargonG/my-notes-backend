...
class InMemoryIntegrationRepository[F[_]: Monad, IN, OUT](
  ...
  override def set(key: IN, value: OUT): F[Long] =
    cacheIn.add(key, value).map(_ => cacheOut.add(value, key)).as(1L)

  override def getByKey(key: IN): OptionT[F, OUT] = OptionT(cacheIn.get(key))

  override def getByValue(value: OUT): OptionT[F, IN] = OptionT(cacheOut.get(value))
